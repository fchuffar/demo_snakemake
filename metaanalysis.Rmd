...
infos2 = lapply(rownames(infos), function(suffix){
  ...
  if (class(infos)=="try-error") {
    ...
    if (is.null(x)) {
      ...
      infos$acme = x$d1       #x$d1.ci, x$d1.p)       #x$z0.ci, x$z0.p))
      infos$ade = x$z0       #x$z0.ci, x$z0.p))
      infos$total_effect =  x$tau.coef #x$tau.ci, x$tau.p))
      infos$prop_mediated =  x$n0       #x$n0.ci, x$n0.p))
      as.data.frame(infos)        
    }
  }
})
...
for (Y in c("simuexectime", "fitmedexectime", "effect1", "effect2", "effect3", "acme", "ade", "total_effect")) {
  for (X in c("ncell", "gamma1", "gamma2", "i")) {
    frm = formula(paste0(Y, "~", X))
    plot(frm, infos, main=frm, col=as.numeric(infos$col))
    legend("topleft", levels(infos$col), pch=1, col=1:length(levels(infos$col)))
  }
}  
```
...
for (Y in c("acme", "ade", "total_effect")) {
  for (X in c("grp")) {
    frm = formula(paste0(Y, "~", X))
